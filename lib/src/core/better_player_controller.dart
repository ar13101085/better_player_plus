...
class BetterPlayerController {
  ...
  Future<void> _handleVideoEvent(VideoEvent event) async {
    switch (event.eventType) {
      ...
      case VideoEventType.tracksChanged:
        // Authoritative track list from the native player — replaces the
        // Dart-side manifest parse, which only knew HLS m3u8 and left
        // DASH sources with empty track lists.
        _applyNativeTracks(event);
  ...
  /// Populate the controller's track caches from a native `tracksChanged`
  /// event. Each entry is shaped per the keys the platform side emits;
  /// see `BetterPlayer.kt::sendTracksChanged` (Android) for the canonical
  /// schema.
  void _applyNativeTracks(VideoEvent event) {
    final videos = event.videoTracks;
    if (videos != null) {
      _betterPlayerAsmsTracks = videos
          .map(
            (m) => BetterPlayerAsmsTrack(
              (m['id'] as String?) ?? '',
              (m['width'] as num?)?.toInt(),
              (m['height'] as num?)?.toInt(),
              (m['bitrate'] as num?)?.toInt(),
              (m['frameRate'] as num?)?.toInt() ?? 0,
              (m['codecs'] as String?) ?? '',
      ...
      if (_betterPlayerAsmsTracks.isNotEmpty) {
        // Match the HLS path — prepend a "default" (auto / ABR) entry so
        // the selector lets the user revert to native track selection.
        _betterPlayerAsmsTracks.insert(0, BetterPlayerAsmsTrack.defaultTrack());
      }
    }
    final audios = event.audioTracks;
    if (audios != null) {
      _betterPlayerAsmsAudioTracks = audios
          .map(
            (m) => BetterPlayerAsmsAudioTrack(
    ...
    // Subtitles intentionally left to the existing
    // `_setupAsmsDataSource` path — DASH text tracks need separate
    // segmented-download plumbing that's out of scope for this patch.
  }
