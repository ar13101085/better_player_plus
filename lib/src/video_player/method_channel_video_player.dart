  ...
  Stream<VideoEvent> videoEventsFor(int? textureId) =>
      _eventChannelFor(textureId).receiveBroadcastStream().map((event) {
        ...
        switch (eventType) {
          ...
          case 'tracksChanged':
            // Coerce the raw `List<dynamic>` of `Map`s coming over the
            // platform channel into a typed `List<Map<String, dynamic>>`
            // each entry shaped per the keys the native side emits.
            List<Map<String, dynamic>> coerce(dynamic raw) => raw is List
                ? raw
            ...
            return VideoEvent(
              ...
              key: key,
              videoTracks: coerce(map['videos']),
              audioTracks: coerce(map['audios']),
              subtitleTracks: coerce(map['subtitles']),
            );
