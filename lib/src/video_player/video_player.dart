...
class VideoPlayerValue {
  ...
  ///
  /// `errorDescription ?? this.errorDescription` means copyWith can never
  /// CLEAR an error — passing null keeps the old one. Every routine event
  /// (position, buffering, initialized) flows through copyWith, so once an
  /// error is latched it survives until the next full [VideoPlayerValue]
  /// reset. [clearError] is the explicit escape hatch for the recovery
  /// paths that need to drop a stale error without resetting everything.
...
class VideoPlayerController extends ValueNotifier<VideoPlayerValue> {
  ...
  /// True while [_setDataSource]'s platform call is in flight. Events (in
  /// particular errors) delivered in this window were generated by the source
  /// being switched away from and must not touch the new source's state.
  bool _settingDataSource = false;
  ...
  Future<void> _create() async {
    ...
    void eventListener(VideoEvent event) {
      ...
      switch (event.eventType) {
        case VideoEventType.initialized:
          // `clearError` — an `initialized` is proof the CURRENT source is
          // alive, so any error still latched on the value is stale: either
          // the previous source's death that crossed the platform channel
          // after _setDataSource's reset (see below), or an error the player
          // has since recovered from by re-preparing. Without this, that
          // stale error can never leave (copyWith keeps it), and
          // BetterPlayerController re-emits an `exception` event on EVERY
          // subsequent value tick — the app tears a perfectly healthy stream
          // down in an endless retry loop.
          value = value.copyWith(duration: event.duration, size: event.size, clearError: true);
        ...
        case VideoEventType.tracksChanged:
          // Pass-through: the BetterPlayerController listens to the same
          // stream and turns this into populated `_betterPlayerAsmsTracks`
          // / `_betterPlayerAsmsAudioTracks` and a `changedTrack` event.
          break;
    ...
    void errorListener(Object object) {
      // Stale by construction — see [_settingDataSource] in [_setDataSource].
      // This error was generated by the source we are switching AWAY from;
      // latching it would poison the new source's value and completer.
      ...
      if (object is PlatformException) {
        ...
        // (was `value.copyWith(...)` with no assignment — a silent no-op that
        // dropped every non-PlatformException error on the floor.)
  ...
  Future<void> _setDataSource(DataSource dataSourceDescription) async {
    ...
    // Everything the event channel delivers while this await is in flight
    // belongs to the PREVIOUS source: the native setDataSource handler stops
    // the old player and only *starts* preparing the new one before replying,
    // so the new source's first real event can only be posted after the reply.
    // In particular the old source's death frequently crosses the platform
    // channel exactly here (the user zaps away from a dead channel) — without
    // the [_settingDataSource] window, `errorListener` stamps that stale error
    // onto this source's fresh value (where copyWith keeps it alive forever,
    // re-emitting an `exception` on every tick) and fails the brand-new
    // [_initializingCompleter] — the next channel is reported dead too.
