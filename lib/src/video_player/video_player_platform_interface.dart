...
class VideoEvent {
  ...
  /// Native-reported video tracks (set only on [VideoEventType.tracksChanged]).
  /// Each entry carries `id`, `width`, `height`, `bitrate`, `frameRate`,
  /// `codecs`, `mimeType` — the raw shape ExoPlayer / AVPlayer exposes.
  final List<Map<String, dynamic>>? videoTracks;

  /// Native-reported audio tracks (set only on [VideoEventType.tracksChanged]).
  /// Each entry: `id` (group index for `setAudioTrack`), `label`,
  /// `language`, `mimeType`.
  final List<Map<String, dynamic>>? audioTracks;

  /// Native-reported subtitle tracks (set only on [VideoEventType.tracksChanged]).
  /// Each entry: `id`, `label`, `language`, `mimeType`.
...
enum VideoEventType {
  ...
  /// The native player reported the authoritative track list (video,
  /// audio, subtitles). Carries `videoTracks`, `audioTracks`,
  /// `subtitleTracks`. Fires on every player track-set change — typically
  /// once after the manifest is parsed, again after DRM session opens.
  tracksChanged,
